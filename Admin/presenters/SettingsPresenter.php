...
class SettingsPresenter extends BasePresenter {
	...
	/* @var \WebCMS\AntiqueModule\Doctrine\Payment */
	private $payment;
	
	/* @var \WebCMS\AntiqueModule\Doctrine\Shipping */
	private $shipping;
	
	/* @var \WebCMS\AntiqueModule\Doctrine\OrderState */
	private $status;
	...
	protected function startup() {
		...
		$this->paymentRepository = $this->em->getRepository('\WebCMS\AntiqueModule\Doctrine\Payment');
		$this->shippingRepository = $this->em->getRepository('\WebCMS\AntiqueModule\Doctrine\Shipping');
		$this->statusRepository = $this->em->getRepository('\WebCMS\AntiqueModule\Doctrine\OrderState');
	}
	...
	public function createComponentSettingsForm(){
		...
		$settings = array();
		$settings[] = $this->settings->get('API identificator', 'antiqueModule', 'text', array());
		$settings[] = $this->settings->get('API token', 'antiqueModule', 'text', array());
		$settings[] = $this->settings->get('Category body class', 'antiqueModule', 'text', array());
		$settings[] = $this->settings->get('Product detail body class', 'antiqueModule', 'text', array());
		$settings[] = $this->settings->get('Order saved email', 'antiqueModule', 'textarea', array());
	...
	public function createComponentPaymentsGrid($name){
		$grid = $this->createGrid($this, $name, '\WebCMS\AntiqueModule\Doctrine\Payment', NULL, array(
	...
	public function actionUpdatePayment($idPage, $id){
		...
		else $this->payment = new \WebCMS\AntiqueModule\Doctrine\Payment;
	}
	...
	public function createComponentShippingsGrid($name){
		$grid = $this->createGrid($this, $name, '\WebCMS\AntiqueModule\Doctrine\Shipping', NULL, array(
				'language = ' . $this->state->language->getId(),
			)
		);
	...
	public function actionUpdateShipping($idPage, $id){
		...
		else $this->shipping = new \WebCMS\AntiqueModule\Doctrine\Shipping;
	}
	...
	public function createComponentStatusesGrid($name){
		$grid = $this->createGrid($this, $name, '\WebCMS\AntiqueModule\Doctrine\OrderState', NULL, array(
				'language = ' . $this->state->language->getId(),
			)
		);
	...
	public function actionUpdateStatus($idPage, $id){
		...
		else $this->status = new \WebCMS\AntiqueModule\Doctrine\OrderState;
	}
	...
	public function createComponentExportSettingsForm(){
		...
		$settings = array();
		$settings[] = $this->settings->get('Save zbozi.cz XML file after product update', 'antiqueModule', 'checkbox');
		$settings[] = $this->settings->get('Save heureka.cz XML file after product update', 'antiqueModule', 'checkbox');
